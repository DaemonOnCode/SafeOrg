...
protopath = "ML/Social_Distance/MobileNetSSD_deploy.prototxt"
modelpath = "ML/Social_Distance/MobileNetSSD_deploy.caffemodel"
...
def main():
    vc = cv2.VideoCapture()
    if not vc.open('ML/Social_Distance/testvideo2.mp4'):
    ...
    vw = cv2.VideoWriter()
    if not vw.open('gosh.mp4', fourcc, fps, size):
